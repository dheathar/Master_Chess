...
classifies a move, never chooses your next action (a fixed rule does that), and
never invents chess advice. If the AI backend is unavailable, every one of these
...
meet them. The left sidebar groups them into the three beats of the loop, so the
navigation itself tells you where you are:
\textbf{Your games} (Games, Library) $\rightarrow$ \textbf{Diagnosis} (Progress,
Player Model) $\rightarrow$ \textbf{Training} (Training Plan, Drills).
